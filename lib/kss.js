...
checkReference = function(paragraphs, options) {
	...
	if (styleKeyRegExp) {
		// Build a regex to remove the "styleguide" keyword (or typo) from the ref.
		styleKeyRegExp += ':?\\s+';
		reference = lastParagraph.replace(new RegExp(styleKeyRegExp, 'i'), '');
		if (reference === lastParagraph) {
		  throw 'Generated styleKeyRegExp, /' + styleKeyRegExp + '/i, has failed.';
		}
